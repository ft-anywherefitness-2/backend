...
router.get("/", (req, res) => {
  Users.getClass()
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Could not fetch classes", error: err.message });
    });
});
...
router.get("/type", (req, res) => {
  ...
  Users.getClassType(type)
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Error fetching type", error: err.message });
    });
});
  ...
  Users.getIntensity(intensity)
    .then((level) => {
      if (level.length > 0) {
        ...
        res
          .status(400)
          .json({ message: "please choose between low, high or medium" });
      }
    })
    .catch((err) => {
      res
...
router.get("/location", (req, res) => {
  ...
  Users.getByLocation(location)
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Error loading data", error: err.message });
    });
});

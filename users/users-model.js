...
function addUser(user) {
  return db.select("*").from("user").insert(user);
}

function getUserById(id) {
  return db.select("*").from("user").where({ id });
}
...
function findBy(user) {
  return db.select("*").from("user").where(user);
}
...
function getClassType(type) {
  return db.select("*").from("class").where("class.type", "=", `${type}`);
}

function getIntensity(intensity) {
  return db.select("*").from("class").where({ intensity });
}

function getByLocation(location) {
  return db.select("*").from("class").where({ location });
}

function getByDuration(duration) {
  return db.select("*").from("class").where({ duration });
}

function getByInstructor(instructor_name) {
  return db.select("*").from("class").where({ instructor_name });
}
